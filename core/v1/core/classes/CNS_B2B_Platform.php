...
class CNS_B2B_Platform
{
	...
	{
		if (!$this->_db->insert('cns_cluster_platform', $fields)) 
	...
	# USER CREATE AN ACCOUNT
	public function insert_platform_product($fields = array())
	{
		if (!$this->_db->insert('cns_cluster_platform_product', $fields)) 
			throw new Exception("There was a problem creating an account.");
	}

	# USER DATA UPDATE
	public function update_platform_product($fields = array(), $id = null)
	{
		if (!$this->_db->update('cns_cluster_platform_product', $id, $fields)) 
			throw new Exception('There was a problem updating');
	}

	# USER CREATE AN ACCOUNT
	public function insert_platform_package($fields = array())
	{
		if (!$this->_db->insert('cns_cluster_platform_package', $fields)) 
			throw new Exception("There was a problem creating an account.");
	}

	# USER DATA UPDATE
	public function update_platform_package($fields = array(), $id = null)
	{
		if (!$this->_db->update('cns_cluster_platform_package', $id, $fields)) 
			throw new Exception('There was a problem updating');
	}
