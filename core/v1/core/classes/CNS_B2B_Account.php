...
class CNS_B2B_Account
{
	...
	{
		if (!$this->_db->insert('cns_cluster_account_b2b', $fields)) 
	...
	{
		if (!$this->_db->update('cns_cluster_account_b2b', $id, $fields)) 
			throw new Exception('There was a problem updating');
	}

	public function insert_branch($fields = array())
	{
		if (!$this->_db->insert('cns_cluster_account_b2b_branch', $fields)) 
			throw new Exception("There was a problem creating an account.");
	}

	#  DATA UPDATE
	public function update_brach($fields = array(), $id = null)
	{
		if (!$this->_db->update('cns_cluster_account_b2b_branch', $id, $fields)) 
			throw new Exception('There was a problem updating');
	}

	public function insert_subscription($fields = array())
	{
		if (!$this->_db->insert('cns_cluster_account_b2b_subscription', $fields)) 
			throw new Exception("There was a problem creating an account.");
	}

	#  DATA UPDATE
	public function update_subscription($fields = array(), $id = null)
	{
		if (!$this->_db->update('cns_cluster_account_b2b_subscription', $id, $fields)) 
			throw new Exception('There was a problem updating');
	}
	...
	public function find($user = null)
	{
		if ($user) {
			...
			if (is_numeric($user)) {
				$field = 'ID';
				$data = $this->_db->get('cns_cluster_account_b2b', array($field, '=', $user));
				if ($data->count()) {
				...
				$field = 'email';
				$data = $this->_db->get('cns_cluster_account_b2b', array($field, '=', $user), $limit = "");
				if ($data->count()) {
					$this->_data = $data->first();
					$hit = true;
				}
	...
	{
		$data = $this->_db->query("SELECT* FROM `cns_cluster_account_b2b` {$sql}", $params);
		if ($data->count()) {
