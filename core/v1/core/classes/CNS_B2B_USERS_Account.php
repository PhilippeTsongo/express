...
class CNS_B2B_USERS_Account
{
	...
	{
		if (!$this->_db->insert('cns_cluster_account_b2b_users', $fields)) 
	...
	{
		if (!$this->_db->update('cns_cluster_account_b2b_users', $id, $fields)) 
			throw new Exception('There was a problem updating');
	}

	public function updateMultiple($fields = array(), $conditions = array()){
		if(!$this->_db->updateMultiple('cns_cluster_account_b2b_users',$conditions, $fields))
			throw new Exception('There was a problem updating');
	}
	...
	public function find($user = null)
	{
		if ($user) {
			...
			if (is_numeric($user)) {
				$field = 'ID';
				$data = $this->_db->get('cns_cluster_account_b2b_users', array($field, '=', $user));
				if ($data->count()) {
				...
				$field = 'email';
				$data = $this->_db->get('cns_cluster_account_b2b_users', array($field, '=', $user), $limit = "");
				if ($data->count()) {
					$this->_data = $data->first();
					$hit = true;
				}
	...
	{
		$table = "cns_cluster_account_b2b_users";
		if ($user) {
			$hit = false;
			if (is_numeric($user)) {
				$field = 'ID';
	...
	{
		$data = $this->_db->query("SELECT* FROM `cns_cluster_account_b2b_users` {$sql}", $params);
		if ($data->count()) {
