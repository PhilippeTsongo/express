                ...
                <td>
                  <div class="btn-group">
                    ...
                    <ul class="dropdown-menu">
                      <li><a href="${Init.DN}package_type/edit/${data.token_id}" class="dropdown-item"> <i class="fa fa-pencil"></i> Edit </a></li>
...
function cns_get_data() {
  ...
  if (response.status === 200) {
    ...
    IOS.putvl(".package-type-name", data.name);
    IOS.putvl(".package-type-description", data.description);
    $('.pmd-textfield').removeClass('pmd-textfield-floating-label');
  } else {
    RedirectURL.to(Init.DN + "package_type/list");
  }
}
...
function smartModalPopUp(paramType, paramID, paramName) {
  ...
  if (paramType == 'activateModal') {
    ...
    modalTitle = 'Package Type Activation';
    modalContent = '<span class="text-center">Do you really want to activate this Package Type ' + paramName + ' ?</span>';

    request = '7nr0EojM+V9sn0MvPty591TO5HaA7Y6qJTCTGbKjPAFTRXx0rMooBNNVD+jWXdai';
    webToken = 'FZrp/HEwJwWCrAH303ypUQ';

    btnAction = 'Activate Package Type';
    status = "ACTIVE";
  }

  if (paramType == 'deactivateModal') {
    requestType = 'admin-deactivate';
    modalTitle = 'Package Type Deactivation';
    modalContent = '<span class="text-center">Do you really want to deactivate this Package Type ' + paramName + ' ?</span>';

    request = '7nr0EojM+V9sn0MvPty596Lcut4pIE5k0rjYeXyhPVdfN7okAr+YPaN3lrhGUbJk';
    webToken = 'FZrp/HEwJwWCrAH303ypUQ';

    btnAction = 'Deactivate Package Type';
    status = "DEACTIVATION";
  }
  ...
  if (paramType == 'deleteModal') {
    ...
    modalContent = '<span class="text-center">Do you really want to delete this Package Type ' + paramName + ' ?</span>';

    request = '7nr0EojM+V9sn0MvPty599FQWJ7sj6mIQ2fOl+FVkMM';
    webToken = 'FZrp/HEwJwWCrAH303ypUQ';

    btnAction = 'Delete Package Type';
    status = "DELETE";
  }
