                ...
                <td>
                  <div class="btn-group">
                    ...
                    <ul class="dropdown-menu">
                      ...
                      <li><a href="${Init.DN}delivery/agent/edit/${data.token_id}" class="dropdown-item"> <i class="fa fa-pencil"></i> Edit </a></li>
...
function cns_get_data() {
  ...
  if (response.status === 200) {
    ...
    IOS.putvl("#firstname", data.firstname);
    IOS.putvl("#lastname", data.lastname);

    IOS.putvl("#identity_number", data.identity_number);
    IOS.putvl("#email", data.email);
    IOS.putvl("#address", data.address);
    IOS.putvl("#telephone", data.telephone);
...
function smartModalPopUp(paramType, paramID, paramName) {
  ...
  if (paramType == 'activateModal') {
    ...
    modalTitle = 'Delivery Agent Activation';
    modalContent = '<span class="text-center">Do you really want to activate this Delivery Agent ' + paramName + ' ?</span>';

    request = 'qFX+tHrEhVFqBqxcyEV1MoDoXNtd7lfffeHkbbzlFSa8UFZ9IO6DsJXUAJunx30S';
    webToken = 'FZrp/HEwJwWCrAH303ypUQ';

    btnAction = 'Activate Delivery Agent';
    status = "ACTIVE";
  }

  if (paramType == 'deactivateModal') {
    requestType = 'admin-deactivate';
    modalTitle = 'Delivery Agent Deactivation';
    modalContent = '<span class="text-center">Do you really want to deactivate this Delivery Agent ' + paramName + ' ?</span>';

    request = 'qFX+tHrEhVFqBqxcyEV1MorbJ27EB9fkvJnAIs6os++pFgE7h+MEZblkXUI9/WPm';
    webToken = 'FZrp/HEwJwWCrAH303ypUQ';

    btnAction = 'Deactivate Delivery Agent';
    status = "DEACTIVATE";
  }

  if (paramType == 'deleteModal') {
    requestType = 'admin-deactivate';
    modalTitle = 'Delete Delivery Agent';
    modalContent = '<span class="text-center">Do you really want to delete this Delivery Agent ' + paramName + ' ?</span>';

    request = 'qFX+tHrEhVFqBqxcyEV1MsBzSmWvJVqYAvdNOY+GO5lbi7dstwoqeL9zgWjzgBiW';
    webToken = 'FZrp/HEwJwWCrAH303ypUQ';

    btnAction = 'Delete Delivery Agent';
    status = "DELETE";
  }
