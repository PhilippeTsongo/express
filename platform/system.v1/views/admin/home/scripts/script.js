...
function cns_get_list() {
    ...
    if (response.status === 200) {
        let count = 0;
        for (const index in response.ship) {
            let ship = response.ship[index];
            let names = ship.name;
            let token = ship.token_auth;
            count++;
            ...
            <tr>
                <td>${ship.code}</td>
                <td>${ship.source_country}</td>
                <td>${ship.destination_country}</td>
                <td> <span class="badge badge-pill badge-${Function.badge_status_bg(ship.status)}"> ${ship.status} </span> </td>

                <td>
                  <div class="btn-group">
                    <button data-toggle="dropdown" class="btn btn-xs btn-default dropdown-toggle">More </button>
                    <ul class="dropdown-menu">
                      <li><a href="${Init.DN}/ship/detail/${ship.token_id}" class="dropdown-item"> <i class="fa fa-eye"></i> Detail </a></li>
                    </ul>
                  </div>
                </td>
...
function cns_get_data() {
    ...
    if (response.status === 200) {
        ...
        IOS.puthtml('#pc_approved_ships', data.pc_approved_ships + '%');
        IOS.puthtml('#pc_initiated_ships', data.pc_initiated_ships + '%');
        IOS.puthtml(".total-ship", data.total_ships);
        IOS.puthtml("#total-customer", data.total_customer);
        IOS.puthtml("#total-partner", data.total_partners);
        IOS.puthtml("#total-user", data.total_user);
