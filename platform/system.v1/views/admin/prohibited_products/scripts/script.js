                ...
                <td>
                  <div class="btn-group">
                    ...
                    <ul class="dropdown-menu">
                      <li><a href="${Init.DN}prohibited/product/edit/${data.token_id}" class="dropdown-item"> <i class="fa fa-pencil"></i> Edit </a></li>
...
function cns_get_data() {
  ...
  if (response.status === 200) {
    ...
    IOS.putvl("#short-description", data.short_description);
    IOS.putvl("#description", data.description);
    $('.pmd-textfield').removeClass('pmd-textfield-floating-label');
  } else {
    RedirectURL.to(Init.DN + "prohibited/product/list");
  }
}
...
function smartModalPopUp(paramType, paramID, paramName) {
  ...
  if (paramType == 'activateModal') {
    ...
    modalTitle = 'Prohibited Product Activation';
    modalContent = '<span class="text-center">Do you really want to activate this Prohibited Product ' + paramName + ' ?</span>';

    request = 'L7tFiyQzEOUgB+m2lotqUOBDbaKBueK8JmDUezxBwFFN9JO3wyk1NHMMSv/bdaWJ';
    webToken = 'FZrp/HEwJwWCrAH303ypUQ';

    btnAction = 'Activate Prohibited Product';
    status = "ACTIVE";
  }

  if (paramType == 'deactivateModal') {
    requestType = 'admin-deactivate';
    modalTitle = 'Prohibited Product Deactivation';
    modalContent = '<span class="text-center">Do you really want to deactivate this Prohibited Product ' + paramName + ' ?</span>';

    request = 'L7tFiyQzEOUgB+m2lotqUBaPZlgE7Ouen540k79dfp3Yz3phO0Kwy28yYmlBBCdG';
    webToken = 'FZrp/HEwJwWCrAH303ypUQ';

    btnAction = 'Deactivate Prohibited Product';
    status = "DEACTIVATION";
  }
  ...
  if (paramType == 'deleteModal') {
    ...
    request = 'L7tFiyQzEOUgB+m2lotqUBaPZlgE7Ouen540k79dfp3uPw4xc7/OmAevLWqqKcTt';
    webToken = 'FZrp/HEwJwWCrAH303ypUQ';
