...
function cns_get_list() {
  ...
  if (response.status === 200) {
    ...
    for (const index in response.data) {
      ...
      let names = data.destination_country;
...
function smartModalPopUp(paramType, paramID, paramName) {
  ...
  if (paramType == 'activateModal') {
    ...
    modalTitle = 'Destination Country Activation';
    modalContent = '<span class="text-center">Do you really want to activate this Destination Country ' + paramName + ' ?</span>';

    request = 'Tl2DakspCD9eiAFbHw7urB/PjtZT1ZIte2uzDTtfN5SlP0b5GQxhVE8W+k8eckxu';
    webToken = 'FZrp/HEwJwWCrAH303ypUQ';

    btnAction = 'Activate Destination Country';
    status = "ACTIVE";
  }

  if (paramType == 'deactivateModal') {
    requestType = 'admin-deactivate';
    modalTitle = 'Destination Country Deactivation';
    modalContent = '<span class="text-center">Do you really want to deactivate this Destination Country ' + paramName + ' ?</span>';

    request = 'Tl2DakspCD9eiAFbHw7urFTm4WcWJTEw/DkT3BRbLrEqW98s4CqXN6o/xkaXMDRq';
    webToken = 'FZrp/HEwJwWCrAH303ypUQ';

    btnAction = 'Deactivate Destination Country';
    status = "DEACTIVATION";
  }

  if (paramType == 'deleteModal') {
    requestType = 'admin-deactivate';
    modalTitle = 'Delete Destination Country';
    modalContent = '<span class="text-center">Do you really want to delete this Destination Country ' + paramName + ' ?</span>';

    request = 'Tl2DakspCD9eiAFbHw7urFh/tS8OtuqRgUVsCJ/ABE/7Kx3djHrBWRE5Bb5iranI';
    webToken = 'FZrp/HEwJwWCrAH303ypUQ';

    btnAction = 'Delete Destination Country';
    status = "DELETE";
  }
...
function get_optiosn_list_countries(token = "") {
  ...
  IOS.puthtml("#destination-country", row_tr_table);
}
