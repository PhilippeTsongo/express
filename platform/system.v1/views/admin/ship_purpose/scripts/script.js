                ...
                <td>
                  <div class="btn-group">
                    ...
                    <ul class="dropdown-menu">
                      ...
                      <li><a href="${Init.DN}ship/purpose/edit/${data.token_id}" class="dropdown-item"> <i class="fa fa-pencil"></i> Edit </a></li>
...
function cns_get_data() {
  ...
  if (response.status === 200) {
    ...
    IOS.putvl("#description", data.description);
    $('.pmd-textfield').removeClass('pmd-textfield-floating-label');
  } else {
    RedirectURL.to(Init.DN + "ship/purpose/list");
  }
}
...
function smartModalPopUp(paramType, paramID, paramName) {
  ...
  if (paramType == 'activateModal') {
    ...
    modalTitle = 'Ship Purpose Activation';
    modalContent = '<span class="text-center">Do you really want to activate this Ship Purpose ' + paramName + ' ?</span>';

    request = 'hWdzbgvdAV5KQpOGLVZSHsxCTCCXhPZAkNP7RKQB95quXrpaFUlzQbbLzDqou3d+';
    webToken = 'FZrp/HEwJwWCrAH303ypUQ';

    btnAction = 'Activate Ship Purpose';
    status = "ACTIVE";
  }

  if (paramType == 'deactivateModal') {
    requestType = 'admin-deactivate';
    modalTitle = ' Ship Purpose Deactivation';
    modalContent = '<span class="text-center">Do you really want to deactivate this Ship Purpose ' + paramName + ' ?</span>';

    request = 'hWdzbgvdAV5KQpOGLVZSHtDhdFXa1X2+tYuuYoQYpSQK+6VHhTLQG95Mxx3/EZKr';
    webToken = 'FZrp/HEwJwWCrAH303ypUQ';

    btnAction = 'Deactivate Ship Purpose';
    status = "DEACTIVATE";
  }
  ...
  if (paramType == 'deleteModal') {
    ...
    modalContent = '<span class="text-center">Do you really want to delete this Ship Purpose ' + paramName + ' ?</span>';

    request = 'hWdzbgvdAV5KQpOGLVZSHmL/HEVd7C4qIQwe84Ec98g';
    webToken = 'FZrp/HEwJwWCrAH303ypUQ';

    btnAction = 'Delete Ship Purpose';
    status = "DELETE";
  }
